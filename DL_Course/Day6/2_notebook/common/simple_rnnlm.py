...
class SimpleRnnlm:
    def __init__(self, vocab_size, wordvec_size, hidden_size):
        ...
        # パラメータの初期化
        embed_W = (rn(V, D) / 100).astype('f') # 小さな値で初期化する
        rnn_Wx = (rn(D, H) / np.sqrt(D)).astype('f') # 簡易版のXavierの初期値
        rnn_Wh = (rn(H, H) / np.sqrt(H)).astype('f')# 簡易版のXavierの初期値
        rnn_b = np.zeros(H).astype('f') 
        affine_W = (rn(H, V) / np.sqrt(H)).astype('f')# 簡易版のXavierの初期値
        affine_b = np.zeros(V).astype('f')

        # レイヤの生成
        self.layers = [
            TimeEmbedding(embed_W), # 単語埋め込みレイヤ
            TimeRNN(rnn_Wx, rnn_Wh, rnn_b, stateful=True), # RNNレイヤ
