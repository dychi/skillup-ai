...
class AttentionBiEncoder:
    ...
    def __init__(self, vocab_size, wordvec_size, hidden_size):
        ...
        # 順方向LSTMの初期パラメータ
        lstm_Wx_f = (rn(D, 4 * H) / np.sqrt(D)).astype('f')
        lstm_Wh_f = (rn(H, 4 * H) / np.sqrt(H)).astype('f')
        lstm_b_f = np.zeros(4 * H).astype('f')
        # 逆方向LSTMの初期パラメータ
        lstm_Wx_b = (rn(D, 4 * H) / np.sqrt(D)).astype('f')
        lstm_Wh_b = (rn(H, 4 * H) / np.sqrt(H)).astype('f')
        lstm_b_b = np.zeros(4 * H).astype('f')     
